        ...
        if (workflowLevelCheck && [1].includes(createDoc[0].nac_status ?? 0)) {
         ...
         console.log(8)
          ...
          console.log(10)
            ...
            // ถ้าเป็นขายทรัพย์สิน (type 5) → ตรวจสอบราคา
            else if ([5].includes(createDoc[0].nac_type ?? 0)) {
              // ถ้ายังไม่มีราคา → ไป 12 (รอกรอกราคาขาย)
              if (createDoc[0].real_price === null || createDoc[0].real_price === undefined) {
                header[0].nac_status = 12;
              } 
              // ถ้าราคา = 0 → เปลี่ยนเป็นตัดทรัพย์สิน
              else if (createDoc[0].real_price === 0) {
                header[0].nac_type = 4; // เปลี่ยนเป็นตัดทรัพย์สิน
                header[0].nac_status = 5; // ไปบัญชี
              } 
              // ถ้ามีราคา > 0 → ไป 15
              else {
                header[0].nac_status = 15; // ไปรอตรวจสอบบัญชี
              }
            }
